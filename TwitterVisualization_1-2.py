...
u_name = pd.DataFrame(fake_users.name.str.split(' ', 1).tolist(),
                      columns = ['first', 'last'])
...
# convert sample document into a list
...
# group by Date, create a count and start
...
trace_2 = go.Scatter(x = userssum.Date,
                    y = userssum.counts,
                    mode = 'lines+markers',
                    marker = dict(size = 16),
...
layout_2 = {"title": "Accounts created 2009-2017", "xaxis": {"zeroline":False}}
...
# filter table to remove all unnecessary data
# and drop all rows with missing values or 0
data = data.filter(['user_id', 'user_key', 'retweet_count'])
data = data[data.retweet_count != 0]
data = data[data.retweet_count.notnull()]

highest_retweeted_users = []

# temp = data[data.retweet_count == data.retweet_count.max()]
# print(type(temp.index))

for i in range(0, 10):
  temp = data[data.retweet_count == data.retweet_count.max()]
  data = data.drop(temp.index, axis = 0)
  highest_retweeted_users.append(temp)
